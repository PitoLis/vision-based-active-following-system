...
tPid pidHW_Tracking;//红外循迹的PID结构体变量
tPid pidFollow;    //定距跟随PID结构体变量
tPid pidMPU6050YawMovement;  //利用6050偏航角进行姿态控制的PID
...
void PID_init()
{
	...
	pidMotor1Speed.Kp=-0.6f;
	pidMotor1Speed.Ki=0.06f;
	pidMotor1Speed.Kd=-0.9f;
	...
	pidMotor2Speed.Kp=-0.6f;
	pidMotor2Speed.Ki=0.06f;
	pidMotor2Speed.Kd=-0.9f;
	...
	pidFollow.target_val=22.50;//定距跟随 目标距离22.5cm
	pidFollow.err=0.0;
	pidFollow.err_last=0.0;
	pidFollow.err_sum=0.0;
	pidFollow.Kp=-0.5;//定距跟随的Kp大小通过估算，根据PID完全输出数据，确定大概大小，然后再调整
